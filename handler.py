...
def write_vendor_red_flag(vendor_id: str, alert_type: str, severity: str, title: str,
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (vendor_id, alert_type, severity, title, explanation, triggering_logic, related_claim_ids),
        )
        conn.commit()
        return {
            "id": cur.lastrowid, "vendor_id": vendor_id, "alert_type": alert_type,
...
def write_fraud_risk_flag_output(vendor_id: str, risk_flag: str, reason: Optional[str] = None) -> dict:
    ...
    try:
        ...
        conn.commit()
        return {"id": cur.lastrowid, "vendor_id": vendor_id, "risk_flag": risk_flag, "reason": reason}
    finally:
        conn.close()
...
Given the claim details, AI fraud signals, and fraud flags below, identify
...
AI fraud signals: {json.dumps(signals, default=str)}
Fraud flags: {json.dumps(flags, default=str)}
